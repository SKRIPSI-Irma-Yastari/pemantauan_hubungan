...
export default function FirstSurveyPage() {
  const router = useRouter()
  
  // Form State - Identitas
  const [kkks, setKkks] = useState("")
  const [year, setYear] = useState("2024")
  const [month, setMonth] = useState("")
  
  // Form State - Kepatuhan Laporan
  const [reportsRequired, setReportsRequired] = useState<string>("")
  const [reportsOnTime, setReportsOnTime] = useState("")
  const [reportsFormat, setReportsFormat] = useState("")
  const [reportsRevision, setReportsRevision] = useState("")
  
  // Form State - Kehadiran Rapat
  const [meetingsCount, setMeetingsCount] = useState<string>("")
  const [attendanceFrequency, setAttendanceFrequency] = useState("")
  const [punctuality, setPunctuality] = useState("")
  const [activityLevel, setActivityLevel] = useState("")
  
  // Form State - Respons Komunikasi
  const [avgResponseTime, setAvgResponseTime] = useState("")
  const [messagesReplied, setMessagesReplied] = useState("")
  const [answerClarity, setAnswerClarity] = useState("")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic Validation
    if (!kkks || !month || !reportsRequired || !reportsOnTime || !reportsFormat || !reportsRevision || 
        !meetingsCount || !attendanceFrequency || !punctuality || !activityLevel || 
        !avgResponseTime || !messagesReplied || !answerClarity || !dataFollowUp) {
      setStatus({ type: 'error', message: "Mohon lengkapi semua field yang tersedia sebelum mengirim survey." })
      return
    }
    ...
    try {
      const { error } = await supabase
        ...
        .insert([
          { 
            ...
            year: parseInt(year), 
            month,
            reports_required: parseInt(reportsRequired),
            reports_on_time: reportsOnTime,
            reports_format: reportsFormat,
            reports_revision: reportsRevision,
            meetings_count: parseInt(meetingsCount),
      ...
      setIsSuccess(true)
      setStatus({ type: 'success', message: "Data survey berhasil dikirimkan." })
      
    } catch (err: any) {
      console.error("Submission Error:", err)
      setStatus({ type: 'error', message: err.message || "Terjadi kesalahan saat mengirim data. Silakan coba lagi." })
    } finally {
      ...
      <main className="max-w-[1000px] mx-auto px-6 py-16 md:py-24 space-y-16">
        ...
        >
          ...
          <p className="text-on-surface-variant font-medium text-lg leading-relaxed max-w-2xl opacity-80 mx-auto md:mx-0">
            Masukan Anda sangat penting untuk membantu kami meningkatkan kualitas koordinasi dan transparansi hubungan kerja sama antar lembaga.
          </p>
        </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
              <div className="space-y-4">
                <label className="flex items-center gap-2 text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-[0.2em]">
                  <Calendar className="h-4 w-4" /> Tahun
                </label>
                <select 
                  value={year}
          ...
          {/* Section 2: Kepatuhan Laporan */}
          <motion.section 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
            className="p-10 bg-surface-container-low/50 backdrop-blur-sm rounded-[2.5rem] border border-outline-variant/10 shadow-sm space-y-10"
          >
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-[10px] font-bold text-primary uppercase tracking-[0.2em]">
                <FileText className="h-4 w-4" /> 02. Kepatuhan Laporan
              </label>
              <h3 className="text-xl font-heading font-extrabold text-on-background tracking-tight">Indikator Pelaporan Operasional</h3>
            </div>

            <div className="space-y-8">
              {/* Question 1 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">1. Berapa jumlah laporan yang wajib dikirim bulan ini?</label>
                <input 
                  type="number" 
                  value={reportsRequired}
                  onChange={(e) => setReportsRequired(e.target.value)}
                  placeholder="Masukkan jumlah laporan"
                  className="w-full h-14 bg-surface-container/50 border-2 border-transparent rounded-2xl px-6 text-sm font-bold focus:border-primary/40 focus:ring-4 focus:ring-primary/5 transition-all outline-none"
                  disabled={isSubmitting}
                />
              </div>
              ...
              {/* Question 2 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">2. Berapa laporan yang dikirim tepat waktu?</label>
                <div className="flex flex-wrap gap-3">
                  {["Semua", "Sebagian", "Tidak Ada"].map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setReportsOnTime(option)}
                      disabled={isSubmitting}
                      className={cn(
                        "flex-1 min-w-[140px] px-6 py-4 rounded-xl text-sm font-bold transition-all border-2",
                        reportsOnTime === option 
              ...
              {/* Question 3 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">3. Apakah format laporan sesuai ketentuan?</label>
                <div className="flex flex-wrap gap-3">
                  {["Selalu Sesuai", "Kadang Sesuai", "Tidak Sesuai"].map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setReportsFormat(option)}
                      disabled={isSubmitting}
                      className={cn(
                        "flex-1 min-w-[140px] px-6 py-4 rounded-xl text-sm font-bold transition-all border-2",
                        reportsFormat === option 
                          ? "bg-primary text-on-primary border-primary shadow-lg" 
                          : "bg-surface-container/30 text-on-surface-variant/40 border-transparent hover:bg-surface-container"
                      )}
                    >
              ...
              {/* Question 4 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">4. Apakah laporan membutuhkan revisi?</label>
                <div className="flex flex-wrap gap-3">
                  {["Tidak Pernah", "Kadang", "Sering"].map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setReportsRevision(option)}
                      disabled={isSubmitting}
                      className={cn(
                        "flex-1 min-w-[140px] px-6 py-4 rounded-xl text-sm font-bold transition-all border-2",
                        reportsRevision === option 
                          ? "bg-primary text-on-primary border-primary shadow-lg" 
                          : "bg-surface-container/30 text-on-surface-variant/40 border-transparent hover:bg-surface-container"
                      )}
                    >
          ...
          >
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-[10px] font-bold text-primary uppercase tracking-[0.2em]">
                <Users className="h-4 w-4" /> 03. Kehadiran Rapat
              </label>
              <h3 className="text-xl font-heading font-extrabold text-on-background tracking-tight">Indikator Koordinasi Tatap Muka</h3>
            </div>

            <div className="space-y-8">
              {/* Question 5 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">5. Berapa kali rapat koordinasi bulan ini?</label>
                <input 
                  type="number" 
                  value={meetingsCount}
                  onChange={(e) => setMeetingsCount(e.target.value)}
                  placeholder="Masukkan jumlah rapat"
                  className="w-full h-14 bg-surface-container/50 border-2 border-transparent rounded-2xl px-6 text-sm font-bold focus:border-primary/40 focus:ring-4 focus:ring-primary/5 transition-all outline-none"
                  disabled={isSubmitting}
                />
              </div>
              ...
              {/* Question 6 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">Berapa kali hadir?</label>
                <div className="flex flex-wrap gap-3">
                  {["Selalu Hadir", "Kadang Hadir", "Jarang Hadir"].map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setAttendanceFrequency(option)}
                      disabled={isSubmitting}
                      className={cn(
                        "flex-1 min-w-[140px] px-6 py-4 rounded-xl text-sm font-bold transition-all border-2",
              ...
              {/* Question 7 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">7. Apakah hadir tepat waktu?</label>
                <div className="flex flex-wrap gap-3">
                  {["Selalu", "Kadang", "Jarang"].map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setPunctuality(option)}
                      disabled={isSubmitting}
                      className={cn(
                        "flex-1 min-w-[140px] px-6 py-4 rounded-xl text-sm font-bold transition-all border-2",
                        punctuality === option 
              ...
              {/* Question 8 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">8. Apakah aktif saat rapat?</label>
                <div className="flex flex-wrap gap-3">
                  {["Aktif", "Cukup Aktif", "Pasif"].map((option) => (
                    <button
                      key={option}
                      type="button"
          ...
          >
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-[10px] font-bold text-primary uppercase tracking-[0.2em]">
                <MessageSquare className="h-4 w-4" /> 04. Respons Komunikasi
              </label>
              <h3 className="text-xl font-heading font-extrabold text-on-background tracking-tight">Indikator Kecepatan & Kualitas Respon</h3>
            </div>

            <div className="space-y-8">
              {/* Question 9 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">9. Rata-rata balasan surat/email?</label>
                <div className="flex flex-wrap gap-3">
                  {["<1 Hari", "1–3 Hari"].map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setAvgResponseTime(option)}
                      disabled={isSubmitting}
                      className={cn(
                        "flex-1 min-w-[140px] px-6 py-4 rounded-xl text-sm font-bold transition-all border-2",
                        avgResponseTime === option 
                          ? "bg-primary text-on-primary border-primary shadow-lg" 
                          : "bg-surface-container/30 text-on-surface-variant/40 border-transparent hover:bg-surface-container"
                      )}
                    >
              ...
              {/* Question 10 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">10. Apakah semua pesan dibalas?</label>
                <div className="flex flex-wrap gap-3">
                  {["Semua", "Sebagian", "Sedikit"].map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setMessagesReplied(option)}
                      disabled={isSubmitting}
                      className={cn(
                        "flex-1 min-w-[140px] px-6 py-4 rounded-xl text-sm font-bold transition-all border-2",
                        messagesReplied === option 
                          ? "bg-primary text-on-primary border-primary shadow-lg" 
                          : "bg-surface-container/30 text-on-surface-variant/40 border-transparent hover:bg-surface-container"
                      )}
                    >
              ...
              {/* Question 11 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">11. Kejelasan jawaban yang diberikan?</label>
                <div className="flex flex-wrap gap-3">
                  {["Sangat Jelas", "Cukup Jelas", "Kurang Jelas"].map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setAnswerClarity(option)}
                      disabled={isSubmitting}
                      className={cn(
                        "flex-1 min-w-[140px] px-6 py-4 rounded-xl text-sm font-bold transition-all border-2",
                        answerClarity === option 
                          ? "bg-primary text-on-primary border-primary shadow-lg" 
                          : "bg-surface-container/30 text-on-surface-variant/40 border-transparent hover:bg-surface-container"
                      )}
                    >
              ...
              {/* Question 12 */}
              <div className="space-y-4">
                <label className="text-sm font-bold text-on-surface-variant">12. Tindak lanjut permintaan data?</label>
                <div className="flex flex-wrap gap-3">
                  {["Cepat", "Sedang", "Lambat"].map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setDataFollowUp(option)}
                      disabled={isSubmitting}
                      className={cn(
                        "flex-1 min-w-[140px] px-6 py-4 rounded-xl text-sm font-bold transition-all border-2",
                        dataFollowUp === option 
